...
async def create_comment(conn, content, parent_id=None):
    ...
    await conn.execute(comments_tree.insert().values(
        ancestor_id=comment_id, descendant_id=comment_id, depth=0
    ))
    if parent_id:
...
async def get_tree(conn, comment_id):
    ...
    query = select([comments_tree.c.depth, comments]).select_from(join) \
        .where(comments_tree.c.ancestor_id == comment_id)
    tree = {}
    rows = await conn.execute(query).fetchall()
    for row in rows:
        make_tree(tree, depth=row[0], comment_id=row[1], content=row[2])
    return tree


def make_tree(tree, comment_id, content, depth):
    if depth == 0:
        tree['id'] = comment_id
        ...
        subtree = {}
        make_tree(subtree, comment_id, content, depth - 1)
